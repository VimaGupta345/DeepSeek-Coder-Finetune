...
class TrainingArguments(transformers.TrainingArguments):
    ...
    run_name: Optional[str] = field(default="deepseek-full-finetune-rust")
...
def process_hf_dataset(data_args: DataArguments):
    ...
    with open(data_args.data_path, "w") as f:
        for ex in train_ds:
            ...
            if not content or len(content) < 100:  # Skip very short code snippets
                continue
...
def load_datasets_from_hf(data_args: DataArguments, training_args: TrainingArguments):
    ...
    # Convert raw code content to instruction/output format
    def convert_to_instruction_format(examples):
        ...
        for content in examples['content']:
            ...
            if len(content) >= 100:  # Filter out very short code snippets
