...
class DataArguments:
    data_path: str = field(
        default="../../../scratch/shared_dir/alpaca_datasets/alpaca_julia.jsonl",
        metadata={"help": "Path to Julia alpaca dataset JSONL file."},
    )
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    run_name: Optional[str] = field(default="deepseek-julia-finetune")
...
def build_instruction_prompt(instruction: str, input_text: str = "") -> str:
    """Build the instruction prompt for Julia code tasks."""
    if input_text.strip():
...
def train_tokenize_function(examples, tokenizer):
    EOT_TOKEN = "<|EOT|>"
    sources = [
        build_instruction_prompt(instruction, input_text)
...
def load_datasets_from_jsonl(data_args: DataArguments, training_args: TrainingArguments):
    """Load train and eval datasets from local JSONL files."""
    import json
    from datasets import Dataset
    
    # Load training dataset from local JSONL
    if training_args.local_rank > 0:
        torch.distributed.barrier()
    
    print(f"Loading Julia training dataset from local file: {data_args.data_path}")
    ...
    print(f"Loaded {len(train_data)} samples from Julia dataset")
    
    # Convert to HuggingFace dataset format
    train_instructions = [item['instruction'] for item in train_data]
    train_inputs = [item.get('input', '') for item in train_data]  # Use get() to handle missing 'input' field
    ...
    if data_args.eval_data_path and data_args.eval_data_path != data_args.data_path:
        # Use separate evaluation dataset
        print(f"Loading evaluation dataset from separate file: {data_args.eval_data_path}")
        eval_data = []
        ...
        eval_inputs = [item.get('input', '') for item in eval_data]
        eval_outputs = [item['output'] for item in eval_data]
        
        eval_raw_dataset = Dataset.from_dict({
            'instruction': eval_instructions,
            'input': eval_inputs,
            'output': eval_outputs
        })
        
        print(f"Using {len(train_data)} samples for training and {len(eval_data)} samples for evaluation")
    else:
        # Create evaluation dataset by splitting the training data
        # Use the last 10% of the training data for evaluation
        eval_size = min(12500, len(train_data) // 10)  # 10% or 12.5k, whichever is smaller
        train_size = len(train_data) - eval_size
        
        print(f"Using {train_size} samples for training and {eval_size} samples for evaluation")
        
        eval_raw_dataset = Dataset.from_dict({
            'instruction': train_instructions[-eval_size:],
            'input': train_inputs[-eval_size:],
            'output': train_outputs[-eval_size:]
        })
        
        # Truncate training dataset to remove evaluation samples
        train_raw_dataset = Dataset.from_dict({
            'instruction': train_instructions[:train_size],
            'input': train_inputs[:train_size],
            'output': train_outputs[:train_size]
        })
...
def train():
    ...
    print(f"Loading from local JSONL file: {data_args.data_path}")
    train_raw_dataset, eval_raw_dataset = load_datasets_from_jsonl(data_args, training_args)
